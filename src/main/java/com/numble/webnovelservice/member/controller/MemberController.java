...
public class MemberController {
    ...
    public ResponseEntity<ResponseMessage<Void>> signUp(MemberSignUpRequest request){
        memberService.signUp(request);
        return new ResponseEntity<>(new ResponseMessage<>("회원가입 성공",null), HttpStatus.CREATED);
    }

    @PostMapping("/auth/login")
    public ResponseEntity<ResponseMessage<Void>> login(MemberLoginRequest request, HttpServletResponse response){
